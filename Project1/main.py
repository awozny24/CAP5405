...
def ClassifyRegressorPredictions(pred, thresh=0.5):
...
def PredictionAccuracy(pred, y=y_test):
...
if (runLR):
    ...
    # predTest, lrModels = TrainAndPredictNextMove(LR())
    lr = LR()
    lr.fit(X_train, y_train)
    predTrain = lr.predict(X_train)
    predTest  = lr.predict(X_test)
    # print(f"Number of Correct Predictions: {sum(y_test[np.array(range(0, y_test.shape[0])), predTest])}")
...
if runKNNR:
    ...
    predTrain = ClassifyRegressorPredictions(predTrain)
    predTest = knnReg.predict(X_test)
    predTest = ClassifyRegressorPredictions(predTest)
...
# print(predTrain)
# # predTest  = lr.predict(X_test)
# # PredictionAccuracy(predTrain, y=y_train)